...
namespace FinanceHub.Api.Features.Salary.Commands.UpdateSalaryProfile;

public record UpdateSalaryProfileCommand(int Id, string Name, string? Description);
...
public class UpdateSalaryProfileCommandHandler(AppDbContext db)
{
    public async Task<SalaryProfileResponse?> HandleAsync(UpdateSalaryProfileCommand command, CancellationToken ct = default)
    {
        ...
        return new SalaryProfileResponse(profile.Id, profile.Name, profile.Description, slipCount);
    }
}
